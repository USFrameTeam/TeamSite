...
// 在文件顶部添加文件加载检查
console.log('script.js loaded, checking files array...');
if (typeof files === 'undefined') {
    console.error('files array is not defined! Check file.js loading order.');
} else {
...
function renderFiles(versionTag = '全部', versionNumber = '全部', searchTerm = '') {
     // 检查files数组是否存在
     if (typeof files === 'undefined') {
        console.error('files array is undefined');
        downloadList.innerHTML = '<div class="error-message">无法加载文件列表，请刷新页面重试</div>';
...
function handleFeedback(baseLink, type, buttonElement) {
    const urls = {
        plugin: 'https://github.com/USFrameTeam/Unknown-Server-Framework-V3-build/issues/new',
